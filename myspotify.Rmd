...
```{r JSON convert, message=FALSE, warning=FALSE, include=FALSE}
...
## Which artist did you spend most time listen to?
...
I also listen to quite a bunch of J-Pop artists. Let's see which is the most listened one. Here are 7 J-Pop artists I have listened to most.
...
Since I'm not sure what a minimum playing hours should be, I will arrange the playing time in descending order and pick top 15 artists. However, you can easily set a minimum threshold by adding a filter.
...
Oh, it's Taylor, again! She definitely rule my Spotify! I just realize that I listened to a lot of game soundtracks. Most of them are from Final Fantasy franchises and NieR series, both are belongs to Square Enix.

## And, who have the most songs?
...
So, it seems that the number of songs has nothing to do with the time listened of an artist. Still, it's quite funny to see Taylor's lonely ring on the top of the graph.

## Most listened songs

I guess apart from artists, you would wonder which songs you got hooked the most. Let's check our top 15 most listened songs.
...
Though Taylor is my most listened artist, 11/15 of my most listened songs were Japanese, with ドライフラワー(Dried Flower) by 優里(Yuuri) and たぶん(Probably) by YOASOBI outweighted other songs.

## During the day, which hours you spent on Spotify most?

We'll make a plot with total hours to have an overall look which part of the day you got hook in Spotify.
...
Oops, what a surprise! I never think I spent most of my time on Spotify during sleeping hours. It might be when I had to worked on night shifts but still, this is quite shock to see. In brief, it's obvious that I used Spotify all day.

## During the week, what day you listened to Spotify most?

You already have a plot of your listening hours during the day, but how about during the week? You spend most of yout Spotify time on weekdays or weekends? Let's try it with a heatmap.
...
As I almost used Spotify non-stop, my time on the app during the week also reflect the same story though I'm sure it's not healthy at all.
...
Using a line plot, it can be seen that I spent a bit more time with Spotify on Friday around 22 o'clock than other days. And, in my unhealthy listening pattern, it looks like Monday is my least favorite day.

# It's time to try Spotifyr package

Spotify provided you your listening data. Great! But did you know that they also have an enormous amount of info such as popularity, energy, valence, how "danceable" a songs is, as well as which album it belongs to.

## Set up

First, set up your Spotify Developer account [here](https://developer.spotify.com/dashboard/) which actually means logging into your current Spotify account. This allows you to make a connection with **Spotify API** and will grant you your `Client Id` and `Client Secret`. In other words, you will use these info to pull your access token into R.
...
The last thing you have to do is to authorize your R analysis with the Spotify.
...
```{r Get access token, include=FALSE}
...
As `spotifyr` can pull the popularity of a song, we can do an analysis to see what are the most popular tracks on your liked songs or a particular playlist. The popularity of a song is measured on the 0-100 scale, with 100 as super hit. Each of your playlist has its own unique id which you have to use Spotify web play to obtain. Or, you can use `get_user_playlists` function to obtain the id of your playlists.
...
Take your time analyze the result 'cause you'll be surprised by the vast information of this data set. As, I have 548 tracks and with the total of 61 variables, there are lots of things I can do. 

Once you've done, let's get back to out initial question *What are the most popular tracks in your liked songs/ playlist?* using `track.popularity` variable. I guess songs with popularity over 80 are pretty much top hits, right?
...
I believe that one's taste of music reflects his/ her personality. Since our `likedSongs` data has some interesting variables such as *valence* and *energy*, I'm curious what my tracks tell about me.

Here are the definition from Spotify regarding these 2 variables:
...
As seen from the plot, most of my favorite songs are on the left side, which means they are sad, depressed, or angry. However, many of those solemn tracks have high energy which can be interpreted as fast, loud, and noisy. Yet, there are still many tracks on the upper right, which represents the 'happy valley'. In brief, my taste of music tells me that I'm quite a negative person. Well, I'm not so sure about it anyway.
